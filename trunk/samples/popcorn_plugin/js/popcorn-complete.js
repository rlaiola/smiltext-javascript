        ...
        if ( that.media.readyState >= 2 ) {
          ...
          that.media.addEventListener( "timeupdate", function( event ) {
            ...
            // Playbar receding
              ...
              while ( tracksByStart[ tracks.startIndex-1 ] && tracksByStart[ tracks.startIndex-1 ].start > currentTime ) {
                // if plugin does not exist on this instance, remove it
                if ( !tracksByStart[ tracks.startIndex-1 ]._natives || !!that[ tracksByStart[ tracks.startIndex-1 ]._natives.type ] ) {
                  if ( tracksByStart[ tracks.startIndex-1 ]._running === true ) {
                    tracksByStart[ tracks.startIndex-1 ]._running = false;
                    tracksByStart[ tracks.startIndex-1 ]._natives.end.call( that, event, tracksByStart[ tracks.startIndex-1 ] );
                  }
                  tracks.startIndex--;
                } else {
                  // remove track event
                  Popcorn.removeTrackEvent( that, tracksByStart[ tracks.startIndex-1 ]._id );
                  return;
                }
              }

              while ( tracksByEnd[ tracks.endIndex-1 ] && tracksByEnd[ tracks.endIndex-1 ].end > currentTime ) {
                // if plugin does not exist on this instance, remove it
                if ( !tracksByEnd[ tracks.endIndex-1 ]._natives || !!that[ tracksByEnd[ tracks.endIndex-1 ]._natives.type ] ) {
                  if ( tracksByEnd[ tracks.endIndex-1 ].start <= currentTime &&
                        tracksByEnd[ tracks.endIndex-1 ]._running === false  &&
                          that.data.disabled.indexOf( tracksByEnd[ tracks.endIndex-1 ]._natives.type ) === -1 ) {

                    tracksByEnd[ tracks.endIndex-1 ]._running = true;
                    tracksByEnd[ tracks.endIndex-1 ]._natives.start.call( that, event, tracksByEnd[tracks.endIndex-1] );
                  }
                  tracks.endIndex--;
                } else {
                  // remove track event
                  Popcorn.removeTrackEvent( that, tracksByEnd[ tracks.endIndex-1 ]._id );
                  return;
                }
              }
            }
